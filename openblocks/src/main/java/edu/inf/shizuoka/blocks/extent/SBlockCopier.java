...
public class SBlockCopier {
	...
	private static RenderableBlock copy(RenderableBlock srcRBlock) {
		...
		for (BlockConnector con : newBlock.getSockets()) {
			if (con.hasBlock()) {
				deleteRecursively(srcBlock.getWorkspace().getEnv().getBlock(con.getBlockID()));
			}
		}
	...
	private static void copyValueAndConnect(BlockConnector srcCon,
		...
		if (srcCon.hasBlock()) {
			RenderableBlock newValueRBlock = copy(newBlock.getWorkspace().getEnv().getRenderableBlock(srcCon.getBlockID()));
			Block newValueBlock = newValueRBlock.getBlock();
			BlockConnector newValueCon = getLeftConnection(newValueBlock);// 最後のつなぐ箇所　注意
			BlockLink link = BlockLink.getBlockLink(newBlock.getWorkspace(),newBlock, newValueBlock,
					newCon, newValueCon);
			link.connect();
			// これをやらないと形が変わらない
			newBlock.getWorkspace().notifyListeners(
					new WorkspaceEvent(newBlock.getWorkspace(),newValueRBlock.getParentWidget(), link,
	...
	private static void deleteRecursively(Block block) {
		...
		for (BlockConnector con : block.getSockets()) {
			if (con.hasBlock()) {
				Block subBlock = block.getWorkspace().getEnv().getBlock(con.getBlockID());
		...
		if (meToOwnerCon.hasBlock()) {
			Block ownerBlock = block.getWorkspace().getEnv().getBlock(meToOwnerCon.getBlockID());
			BlockConnector ownerToMeCon = getRightConnection(ownerBlock, block);
			BlockLink.getBlockLink(ownerBlock.getWorkspace(), ownerBlock, block, ownerToMeCon,
					meToOwnerCon).disconnect();
		}

		// delete me
		BlockUtilities.deleteBlock(block.getWorkspace().getEnv().getRenderableBlock(block
				.getBlockID()));
	}
	...
	public static void copyRecursivelyByEmuration(RenderableBlock rb,
		...
		for (BlockConnector con : newRb.getBlock().getSockets()) {
			deleteRecursivelyByHandEmuration(
					rb.getBlock().getWorkspace().getEnv().getRenderableBlock(con.getBlockID()), e);
		}
		...
		rb = rb.getBlock().getWorkspace().getEnv().getRenderableBlock(next);
