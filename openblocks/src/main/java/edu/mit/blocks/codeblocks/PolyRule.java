...
//		}
//	}
		ProcedureOutputManager procedureOutputManager;
	   ...
	   public PolyRule(Workspace ws)
	    ...
	    private static void setPolyConnectors(Workspace ws, BlockConnector setterConn, Long setterBlockID)
	    {
	        Block otherBlock = ws.getEnv().getBlock(setterConn.getBlockID());
	    ...
	    private static void setPolyKind(Block polyBlock, BlockConnector polyConn, String newType)
	    {
	        ...
	        if(polyConn.getKind().startsWith("poly-"))
	        {
	            prefix = (new StringBuilder()).append(prefix).append(polyConn.getKind().substring(4)).toString();
	            polyConn.setKind(prefix);
	        }
	    }
	    ...
	    private static boolean canRevertPolyConnectors(Workspace ws, Block b, Long prevBlock)
	    {
	        ...
	        for(Iterator i$ = getPolyConnectors(b).iterator(); i$.hasNext();)
	        {
	            ...
	            if(polyConn.hasBlock() && polyConn.getBlockID() != prevBlock)
	            {
	                ...
	                if(!isInitPoly(otherBlock.getConnectorTo(b.getBlockID())) || !canRevertPolyConnectors(ws, otherBlock, b.getBlockID()))
	                    return false;
	            }
	        }
	    ...
	    {
	        if(!isInitPoly(polyConn) || !canRevertPolyConnectors(polyBlock.getWorkspace(), polyBlock, Block.NULL))
	            return;
	    ...
	    private static void revertPolyConnectors(Workspace ws, BlockConnector setterConn, Long setterBlockID)
	    {
	        Block otherBlock = ws.getEnv().getBlock(setterConn.getBlockID());
	    ...
	    private static void revertPolyKind(BlockConnector polyConn, BlockConnector setterConn)
	    {
	        ...
	        if(polyConn.getKind().contains("list"))
