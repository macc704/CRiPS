 ...
 * 		・jdk1.7でsoundが動かない問題をfix
 * version 1.5.12 2012/10/09
 * 		・最近のJava(1.7)でappletでalwaysOnTopが発動されると，起動しない問題 
 * 		・TurtleFrame if (JavaVertionChecker.getMinorVersion() >= 5 && && !TurtleLaunchApplet.initialized) {
 * 		・JFrame.EXIT_ON_CLOSEもJDK1.7で禁止になる．
 * 			174行目修正 if (!TurtleLaunchApplet.initialized) {
 * 			window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
 * version 1.5.13 2012/11/06
 * 		・BEに対応するため，SoundTurtleの仕様変更
 * version 1.5.14 2012/11/07
 * 		・BEに対応するため，setShow()を追加
 * version 1.5.15 2012/11/14
 * 		・Java Web Startに対応するためImageTurtleのリソース取得方法を変更
 * version 1.5.16 2012/11/14
 * 		・引数に"jnlp"を渡すとコンソールが表示されるように変更
 * 		・Java Web Startで署名がなくても動作するように変更
 * version 1.5.17 2012/11/27
 * 		・JNLPMainを作成
 * version 1.5.18 2012/11/28
 * 		・Java Web Start時の不具合を修正
 * version 1.5.23 2012/12/19
 * 		・Java Web Start時のEXIT_ON_CLOSEにならない不具合を修正
 * 		・Java Web Start時のコンソールを別ウインドウに変更
 * version 1.5.24 2013/01/09
 *      ・DEFAULT_IS_ALWAYS_ON_TOP をfalseに
 *      ・起動時にtoFront();
