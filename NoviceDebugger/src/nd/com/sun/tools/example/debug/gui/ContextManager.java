/*
 * @(#)ContextManager.java	1.16 05/11/17
 *
...
import nd.com.sun.tools.example.debug.bdi.FrameIndexOutOfBoundsException;
import nd.com.sun.tools.example.debug.bdi.SessionListener;
import nd.com.sun.tools.example.debug.bdi.ThreadInfo;
...
public class ContextManager {
	...
	public void setCurrentThreadInvalidate(ThreadReference t) {
		currentThread = t;
		notifyCurrentFrameChanged(runtime.threadInfo(t), 0, true);
	}
	...
	/******
	 * public int getCurrentFrameIndex() { return
	 * getCurrentFrameIndex(currentThreadInfo); }
	 ******/
	...
	// ### to the current thread and current frame. When the current thread is
	// set automatically
	// ### after a breakpoint hit or step completion, no event is generated.
	// Instead,
	// ### interested parties are expected to listen for the BreakpointHit and
	// StepCompleted
	// ### events. This convention is unclean, and I believe that it reflects a
	// defect in
	// ### in the current architecture. Unfortunately, however, we cannot
	// guarantee the
	// ### order in which various listeners receive a given event, and the
	// handlers for
	// ### the very same events that cause automatic changes to the current
	// thread may also
	...
	private void notifyCurrentFrameChanged(ThreadInfo tinfo, int index,
			boolean invalidate) {
		Vector<ContextListener> l = (Vector<ContextListener>) contextListeners
				.clone();
	...
	/**
	 * Add a -classpath argument to the arguments passed to the exec'ed VM with
	 * the contents of CLASSPATH environment variable, if -classpath was not
	 * already specified.
	 * 
	 * @param javaArgs
	 *            the arguments to the VM being exec'd that potentially has a
	 *            user specified -classpath argument.
	...
	/*
	 * private String appendPath(String path1, String path2) { if (path1 == null
	 * || path1.length() == 0) { return path2 == null ? "." : path2; } else if
	 * (path2 == null || path2.length() == 0) { return path1; } else { return
