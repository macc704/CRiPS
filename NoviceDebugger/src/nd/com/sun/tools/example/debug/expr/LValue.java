...
package nd.com.sun.tools.example.debug.expr;
...
abstract class LValue {
	...
	static Field fieldByName(ReferenceType refType, String name, int kind) {
		...
		/***
		 * System.err.println("fieldByName: " + refType.name() + " " + name +
		 * " " + kind + " " + (field != null));
		 ***/
		return field;
	}
	...
	static LValue makeNewObject(VirtualMachine vm,
			...
			List arguments) throws ParseException {
				...
				while (izer.hasMoreTokens()) {
					...
					if (classes.size() > 0) {
						if (classes.size() > 1) {
							...
							ReferenceType refType = (ReferenceType) classes
									.get(0);
