			...
			while (conn.established()) {
				...
				if (obj instanceof Integer) { // Integerなら部屋番号かスクロールバーのvalue
					if ((int) obj < 0) { // マイナスなら部屋番号
						final int roomNum = (int) obj;
						System.out.println("final roomNum is " + roomNum);
						if (!divideRoom.checkRoomNum(roomNum * (-1))) {
							pools.add(new ConnectionPool());
							System.out.println("create new ConnectionPool");
						}
						connectionPool = pools.get(divideRoom.countRoom(roomNum
								* (-1)));
