...
public class InfixBlockShape extends BlockShape {
	...
	protected void makeBottomSide() {
		...
		for (BlockConnector curSocket : block.getSockets()) {
			...
			if (curSocket.getPositionType() == BlockConnector.PositionType.BOTTOM) {
				...
				if (socketCounter > 0) {
					gpBottom.lineTo((float) gpBottom.getCurrentPoint().getX() + BOTTOM_SOCKET_MIDDLE_SPACER,
							(float) gpBottom.getCurrentPoint().getY());
				} else {
					gpBottom.lineTo((float) gpBottom.getCurrentPoint().getX() + BOTTOM_SOCKET_SIDE_SPACER,
							(float) gpBottom.getCurrentPoint().getY());
				}
				...
				if (curSocket.getBlockID() == Block.NULL) {
					...
					double unzoomX = connectedRBlock.getSocketPixelPoint(connectedBlock.getPlug()).getX()
							/ connectedRBlock.getZoom();
					double unzoomY = connectedRBlock.getSocketPixelPoint(connectedBlock.getPlug()).getY()
							/ connectedRBlock.getZoom();
					Point2D connectedBlockSocketPoint = new Point2D.Double(unzoomX, unzoomY);
					Point2D currentPoint = gpBottom.getCurrentPoint();
					double newX = connectedBlockSocketPoint.getX()
							+ Math.abs(connectedBlockSocketPoint.getX() - currentPoint.getX());
					double newY = connectedBlockSocketPoint.getY()
							+ Math.abs(connectedRBlock.getBlockHeight() / connectedRBlock.getZoom()
									- currentPoint.getY());
					rb.updateSocketPoint(curSocket, new Point2D.Double(newX, newY));

					BlockShape connectedBlockShape = RenderableBlock.getRenderableBlock(curSocket.getBlockID())
							.getBlockShape();
				...
				if (socketCounter < block.getNumSockets() - 1) {
					gpBottom.lineTo( // need to add the width of the block label. warning: this assumes that there is
										// only one block label
							(float) gpBottom.getCurrentPoint().getX() + BOTTOM_SOCKET_MIDDLE_SPACER
									+ rb.accomodateLabelsWidth(), (float) gpBottom.getCurrentPoint().getY());
				} else {
					gpBottom.lineTo((float) gpBottom.getCurrentPoint().getX() + BOTTOM_SOCKET_SIDE_SPACER,
							(float) gpBottom.getCurrentPoint().getY());
				}
	...
	private void appendRightSidePath(GeneralPath gpBottom, Block connectedBlock, BlockShape connectedBlockShape) {
		...
		for (BlockConnector socket : connectedBlock.getSockets()) {
			...
			if (socket.getBlockID() == Block.NULL) {
				...
				gpBottom.lineTo((float) gpBottom.getCurrentPoint().getX() + BOTTOM_SOCKET_SIDE_SPACER, (float) gpBottom
						.getCurrentPoint().getY());

				// now draw the empty right socket side
				// draw first socket - down right side
				BCS.addDataSocket(gpBottom, socket.getKind(), false);
				// lastBottomPathWidth = (int)BOTTOM_SOCKET_SIDE_SPACER;
			} else {
				...
				if (block.getNumSockets() == 0 || block.isInfix()) {
					...
					// instead of just appending the right side...draw line to
	...
	protected int determineBlockWidth() {
			...
			if (socket.getPositionType() == BlockConnector.PositionType.BOTTOM) {
				if (socket.getBlockID() == Block.NULL) {
					...
					bottomSocketWidth += BOTTOM_SOCKET_SIDE_SPACER;
							...
							// get their assigned width from rb
					...
					bottomSocketWidth -= BlockConnectorShape.getDataPlugWidth(socket);
		...
		bottomSocketWidth += 2 * BOTTOM_SOCKET_MIDDLE_SPACER; // need to decide for a size of the middle spacer and how
																// to place them
	...
	private void appendPath(GeneralPath gp1, GeneralPath gp2, boolean reversed) {
		ArrayList<Number[]> points = new ArrayList<Number[]>(); // Each element is an array consisting of one Integer
																// and six Floats
		...
		while (!i.isDone()) {
			...
			points.add(new Number[] { new Integer(type), new Float(segment[0]), new Float(segment[1]),
					new Float(segment[2]), new Float(segment[3]), new Float(segment[4]), new Float(segment[5]) });
		}
