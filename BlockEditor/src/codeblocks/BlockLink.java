 ...
 * 
 * Each BlockLink instance stores the Block IDs of the "plug" and "socket" and
 * two block connectors, one from each, with a possible connection.
 * 
 * In block linking, a "plug" can be either a before or plug connector, while a
 * "socket" can either be a after or socket connector. Plugs can only connect to
 * other socket connectors, while before connectors can connect to after and
 * command socket connectors.
 */
public class BlockLink {
	 ...
	 * @param socket2
	 */
	private BlockLink(Block block1, Block block2, BlockConnector socket1,
	...
	/**
	 * This method actually connects the two blocks stored in this BlockLink
	 * object.
	 * 
	 */
	public void connect() {
	 ...
	 * 
	 * @param block1
	 *            one of the Block objects in the potential link
	 * @param block2
	 *            the other Block object
	 * @param socket1
	 *            the BlockConnector from block1
	 * @param socket2
	 *            the BlockConnector from block2
	 * @return a BlockLink object storing the potential link between block1 and
	 *         block2
	 */
	public static BlockLink getBlockLink(Block block1, Block block2,
		...
		if (!((block1.getBlockID().equals(lastPlugID)
				&& block2.getBlockID().equals(lastSocketID)
				&& socket1.equals(lastPlug) && socket2.equals(lastSocket)) || (block2
				.getBlockID().equals(lastPlugID)
				&& block1.getBlockID().equals(lastSocketID)
