...
/**
 * A BlockCanvas is a container of Pages and is a scrollable panel. When a page
 * is added to a BlockCanvas, that particular new page must be added to both the
 * data structure holding the set of pages and the scrollable panel that renders
 * the page.
 * 
 * A BlockCanvas is also a PageChangeListener. When any pages are changed, the
...
public class BlockCanvas implements PageChangeListener, ISupportMemento {
	...
	/**
	 * The scrollable JComponent representing the graphical part of this
	 * BlockCanvas
	 */
	...
	/**
	 * Constructs BlockCanvas and subscribes this BlockCanvas to PageChange
	 * events
	 */
	public BlockCanvas() {
		...
		this.scrollPane = new CHoverScrollPane(canvas,
				...
				ScrollPolicy.HORIZONTAL_BAR_ALWAYS, 18, CGraphite.blue, null);//canvasのスクロールパネルのインスタンス生成
																				//縦横のスクロールバーを常に表示、色は青
	...
	/** @returns vertical scroll bar bounding range b2j.model. MAY BE NULL */
	public BoundedRangeModel getVerticalModel() {
		return scrollPane.getVerticalModel();
	}

	/** @returns horizontal scroll bar bounding range b2j.model. MAY BE NULL */
	public BoundedRangeModel getHorizontalModel() {
		return scrollPane.getHorizontalModel();
	}

	/**
	 * @returns the Swing Container that holds all the graphical panels of all
	 *          the pages in this Blockcanvas
	 */
	...
	public List<Page> getLeftmostPages(int left) {
		...
		for (Page p : this.pages) {
			...
			if (pagePosition >= scrollPosition) {
				if (pagePosition - p.getJComponent().getWidth()
						- scrollPosition <= left - 10) {
					leftmostPages.add(p);
				}
			}
		}
	...
	/**
	 * @return the RendearbleBlocks that are contained within this widget or an
	 *         empty Iterable if no blocks exists
	 */
	...
	/**
	 * @param block
	 *            - the RenderableBlock to make sure is shown in the viewport
	 * @requires block ! null
	 * @modifies the vertical and horizontal scrollbal boundedRangeModel
	 * @effects This method causes the workspace to scroll if needed to complete
	 *          show the given RenderableBlock.
	 */
	 ...
	 * 
	 * @requires none
	 * @return FIRST page with matchng name (if more than one page has matching
	 *         name, it returns the first) or null if no matching name exists.
	 */
	...
	/**
	 * @param page
	 *            the page to add to the BlockCanvas
	 * 
	 * @requires page != null
	 * @modifies this.pages
	 ...
	 * @requires none
	 * @modifies this.pages
	 * @effects Inserts the specified page at the specified position. Shifts the
	 *          element currently at that position (if any) and any subsequent
	 *          elements to the right (adds one to their current position).
	 * @throws RuntimeException
	 *             if (position < 0 || position > pages.size() || page == null)
	 */
	 ...
	 * @requires page != null
	 * @modifies this.pages
	 * @effects Removes the given Page from the BlockCanvas. If specified page
	 *          not found in BlockCanvas, do nothing. if more than one page
	 *          equals the specified page, then remove the first equal()
	 *          instance.
	 */
	...
	/**
	 * @param position
	 *            - 0 is the left most page
	 * 
	 * @requires none
	 * @returns the page that was removed or null if non was removed.
	 * @modifies this.pages
	 * @effects If the position is within bounds, then remove the page located
	 *          at the specified position. Do nothing if the position is out of
	 *          bounds.
	 */
	...
	/**
	 * @param page
	 *            the desired page to switch view to
	 * 
	 * @requries page != null
	...
	/**
	 * @param oldName
	 *            - the original name of the page
	 * @param newName
	 *            - the String name to rename the page to
	 * 
	 * @requires oldName != null && newName != null
	 * @return If a matching page was found, return the renamed Page. Otherwise,
	 *         return null.
	 ...
	 * @effects resynchronize b2j.model and view, resize, reposition, and set
	 *          color of EVERY page in the BlockCanvas. Then move very divider
	 *          to the far right side of it's corresponding page. Note that
	...
	public void reformBlockCanvas() {
		...
		for (PageDivider d : dividers) {
			d.setBounds(d.getLeftPage().getJComponent().getX()
					+ d.getLeftPage().getJComponent().getWidth() - 3, 0, 5, d
					.getLeftPage().getJComponent().getHeight());
		}
		canvas.setPreferredSize(new Dimension(widthCounter,
				(int) (Page.DEFAULT_ABSTRACT_HEIGHT * Page.getZoomLevel())));
		scrollPane.revalidate();
		scrollPane.repaint();
	}
	...
	/**
	 * Returns an XML String describing all the blocks and pages within the
	 * BlockCanvas
	 */
	public String getSaveString() {
		...
		if (pages.size() > 0) { //ria just do BLOCKS, CHECK OUT HOW SAVING WILL BE LIKE WITH REFACTORING
	...
	/**
	 * Loads all the RenderableBlocks and their associated Blocks that reside
	 * within the block canvas. All blocks will have their nessary data
	 * populated including connection information, stubs, etc. Note: This method
	 * should only be called if this language only uses the BlockCanvas to work
	 * with blocks and no pages. Otherwise, workspace live blocks are loaded
	 * from Pages.
	 * 
	 * @param root
	 *            the Document Element containing the desired information
	 */
	protected void loadSaveString(Element root) {
		...
		PageDrawerLoadingUtils.loadPagesAndDrawers(root, Workspace
				.getInstance().getFactoryManager());
	...
	/**
	 * The graphical representation of the block canvas's Swng Container of
	 * pages. Note that this is not the graphical scrollable JComponent that
	 * represents the BlockCanvas.
	 */
	public class Canvas extends JLayeredPane implements MouseListener,
		...
		public void mouseClicked(MouseEvent e) {
			if (SwingUtilities.isRightMouseButton(e) || e.isControlDown()) {
				//pop up context menu
				JPopupMenu popup = ContextMenu
						.getContextMenuFor(BlockCanvas.this);
