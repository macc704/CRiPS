			...
			if(sourceBlock.getPlug() != null && "object".equals(sourceBlock.getPlug().getKind())){
				...
				for(RenderableBlock rb : Workspace.getInstance().getBlockCanvas().getBlocks()){
					...
					for(BlockConnector socket : b.getSockets()){
						System.out.println("plug:" + sourceBlockPlugType + " socket:" + socket.getConnectorJavaType() + " result:" + Workspace.getInstance().getWorkSpaceController().getClassRelationMap().couldConnect(sourceBlockPlugType, socket.getConnectorJavaType()));
