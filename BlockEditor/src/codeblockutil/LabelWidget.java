...
public abstract class LabelWidget extends JComponent {
	...
	/** Border of textfield */
	private static final Border textFieldBorder = new CompoundBorder(
			BorderFactory.createLoweredBevelBorder(), new EmptyBorder(1, 2, 1,
					2));
	/** Number formatter for this label */
	private static final NumberFormatter nf = new NumberFormatter(
			NumberFormatter.MEDIUM_PRECISION);

	/** Label that is visable iff editingText is false */
	private final ShadowLabel textLabel = new ShadowLabel();
	//以下テスト用，#matsuzawa 2012.10.10
	//	private final ShadowLabel textLabel = new ShadowLabel() {
	...
	/**
	 * created by sakai lab 2011/10 関数のときは、ラベルにアルファベットもしくはアンダーバーしか使えないようにする。
	 */
	...
	public LabelWidget(String initLabelText, Color fieldColor,
		...
		if (initLabelText == null)
			initLabelText = "";
		this.setFocusTraversalKeysEnabled(false);// MOVE DEFAULT FOCUS TRAVERSAL
		// KEYS SUCH AS TABS
		...
		textField.setSelectedTextColor(Color.WHITE);// white text when
		// highlighted
	...
	public void setEditingState(boolean editing) {
		if (editing) {
			editingText = true;
			// textField.setText(textLabel.getText().trim()); // original
			String labelText = trim(textLabel.getText());
			textField.setText(labelText);
			labelBeforeEdit = labelText;
			this.removeAll();
			this.add(textField);
			textField.grabFocus();
			// textField.setText(labelBeforeEdit);
		} else {
			// update to current textfield.text
			// if text entered was not empty and if it was editing before
			if (editingText) {
				// make sure to remove leading and trailing spaces before
				// testing if text is valid
				// if allow labels to have leading and trailing spaces,
				// will need to modify this if statement
				// if (isTextValid(textField.getText().trim())) {
				if (isTextValid(textField.getText())) {
					// String editLabel = textField.getText().trim();
					String editLabel = textField.getText();
					// arranged by sakai lab 2011/10
					if (!resolveLabel(editLabel)) {
	...
	//added by sakai lab #matsuzawa 2012.10.21 多少adhocな修正？ 2.3.1
	...
	public void setVariable(boolean isVariable) {
		...
		if (isVariable) {
			textField.setVariable(isVariable);
		}
	}
	...
	public void setProcedure(boolean isProcedure) {
		...
		if (isProcedure) {
			textField.setProcedure(isProcedure);
		}
	}
	...
	public String getText() {
		// return textLabel.getText().trim(); //original
		return trim(textLabel.getText());// sakai lab
	}
	...
	protected void suggestEditable(boolean suggest) {
		if (isEditable) {
			if (suggest) {
				setBorder(BorderFactory.createLineBorder(Color.white));// show
				// white
				// border
			} else {
	...
	public void setZoomLevel(double newZoom) {
		this.zoom = newZoom;
		Font renderingFont;// = new Font(font.getFontName(), font.getStyle(),
		// (int)(font.getSize()*newZoom));
	...
	public String toString() {
		return "Label at " + this.getLocation() + " with text: \""
				+ textLabel.getText() + "\"";
	}
	...
	public boolean canProcessNegativeSign() {
		if (this.getText() != null && this.getText().contains("-")) {
			...
			if (textField.getSelectedText() != null
					&& textField.getSelectedText().contains("-")) {
				return true;
			} else {
				return false;
			}
	 ...
	 */
	// TODO ラベルの入力文字に関する変更はここで
	private class BlockLabelTextField extends JTextField implements
			MouseListener, DocumentListener, FocusListener, ActionListener {
		 ...
		 * block and variable block
		 */
		private final char[] validChar = { '1', '2', '3', '4', '5', '6', '7',
		...
		/**
		 * If this is a number, then only allow negative signs and periods at
		 * certain spots
		 */
		private boolean isNumber = false;

		/**
		 * created by sakai lab 2011/10 If this is a variable, then only allow
		 * negative signs and periods at certain spots
		 */
		private boolean isVariable = false;

		/**
		 * created by sakai lab 2011/11/20 If this is a created instance, then
		 * only allow negative signs and periods at certain spots
		 */
		private boolean isNewObject = false;
		...
		private BlockLabelTextField() {
			 ...
			 * key events for focus traversal keys.
			 */
	...
	private class LabelMenu extends JPanel implements MouseListener,
		...
		/**
		 * @param siblings
		 *            = array of siblin's genus and initial label { {genus,
		...
		public void paint(Graphics g) {
			...
			if (isFocused) {
				...
				triangle.lineTo(this.getWidth() / 2 - 1, this.getHeight() / 4
	 ...
	 */
	private class ShadowLabel extends JLabel implements MouseListener,
			MouseMotionListener {
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < shadowPositionArray.length; i++) {
				...
				g2.drawString(this.getText(), (int) ((4 + dx) * offsetSize),
						this.getHeight() + (int) ((dy - 6) * offsetSize));
			}
