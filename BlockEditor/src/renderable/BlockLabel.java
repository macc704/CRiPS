...
public class BlockLabel implements MouseListener, MouseMotionListener,
	...
	public final static Font blockFontSmall_Bold = new Font(fontName,
			Font.BOLD, 7);
	public final static Font blockFontMedium_Bold = new Font(fontName,
			Font.BOLD, 10);
	public final static Font blockFontLarge_Bold = new Font(fontName,
			Font.BOLD, 12);
	public final static Font blockFontSmall_Plain = new Font(fontName,
			Font.PLAIN, 7);
	public final static Font blockFontMedium_Plain = new Font(fontName,
			Font.PLAIN, 10);
	public final static Font blockFontLarge_Plain = new Font(fontName,
			Font.PLAIN, 12);
	 ...
	 */
	public BlockLabel(String initLabelText, BlockLabel.Type labelType,
			boolean isEditable, Color tooltipBackground) {
		// call other constructor
		this(initLabelText, labelType, isEditable, -1, false, tooltipBackground);
	}

	public BlockLabel(String initLabelText, BlockLabel.Type labelType,
			boolean isEditable, long blockID, boolean hasComboPopup,
		...
		widget = new LabelWidget(initLabelText, Block.getBlock(blockID)
				.getColor().darker(), tooltipBackground) {
	...
	public void setText(String text) {
		if (text != null && !text.equals(widget.getText())) {//2012.10.23 余計なBLOCK_CHANGEDイベントが飛ぶ． #matsuzawa
			widget.setText(text);
		}
	}
	...
	protected void textChanged(String text) {
		if ((this.labelType.equals(BlockLabel.Type.NAME_LABEL) || this.labelType
			...
			// we're connected to has stubs, update them.
			if (plug != null && plug.getBlockID() != Block.NULL) {
				if (Block.getBlock(plug.getBlockID()) != null) {
					if (Block.getBlock(plug.getBlockID())
							.isProcedureDeclBlock()
							&& Block.getBlock(plug.getBlockID()).hasStubs()) {
						// Blocks already store their socket names when saved so
						// it is not necessary nor desired to call the connectors changed event again.
						if (RenderableBlock.getRenderableBlock(
								plug.getBlockID()).isLoading()) {
							BlockStub
									.parentConnectorsChanged(plug.getBlockID());
						}
					}
				}
			}
			...
			if (rb != null) {
				Workspace.getInstance().notifyListeners(
						new WorkspaceEvent(rb.getParentWidget(), blockID,
	...
	public void mouseClicked(MouseEvent e) {
		if (!((e.getClickCount() == 1) && widget.isEditable())) {
			RenderableBlock.getRenderableBlock(blockID).processMouseEvent(
