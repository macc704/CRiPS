...
public class ScopeChecker {
	...
	public static boolean checkScope(Block beforeBlock, Block cmpBlock) {
		...
		if (isCompareBlock(cmpBlock) && !cmpBlock.getGenusName().contains("private")) {
			...
			if (compareBlock.getGenusName().equals("callActionMethod2")
				...
				compareBlock = Block.getBlock(compareBlock.getSocketAt(0).getBlockID());
				// 参照ブロックがなかった場合、チェックする参照ブロックがないためtrue privateもtrue
				if (compareBlock == null || compareBlock.getGenusName().contains("private")) {
					return true;// とりあえずくっつける
				}
			}

			// 参照ブロックの名前を取得
			compareBlockName = compareBlock.getBlockLabel().substring(0,
					compareBlock.getBlockLabel().indexOf(compareBlock.getLabelSuffix()));

			// 所属できる場所かどうか確認する
			if (confirmCompareBlockIsBelongable(cmpBlock, beforeBlock, compareBlockName)) {
				return true;
			}

			RenderableBlock.getRenderableBlock(cmpBlock.getBlockID()).setBlockHighlightColor(Color.RED);
	...
	// if (originBlock.getBlockLabel().equals(compareBlockName))
	// {//ラベルを確認して、一致するか
	...
	private static boolean confirmCompareBlockIsBelongable(Block cmpBlock, Block beforeBlock, String originBlockName) {
		...
		while (cmpBlock != null) {
			...
			// procedureだった場合
			if (RenderableBlock.getRenderableBlock(cmpBlock.getBlockID()).getGenus().equals("procedure")) {
				for (BlockConnector socket : cmpBlock.getSockets()) {
					if (socket.getBlockID() != -1) {
						if (Block.getBlock(socket.getBlockID()).getBlockLabel().equals(originBlockName)) {
							return true;
						}
					}
				}
	...
	/*
	 * private boolean confirmCompareBlockIsBelongableAtSockets(Block
	 * originBlock, Block compareBlockParent) { for (BlockConnector socket :
	 * BlockLinkChecker .getSocketEquivalents(originBlock)) { for (Block block =
	 * Block.getBlock(socket.getBlockID()); block != null; block = Block
	 * .getBlock(block.getAfterBlockID())) { if (compareBlockParent.getBlockID()
	 * == block.getBlockID()) {// return true; } if (block.getSockets() != null)
	 * {//ソケットにブロックを持っている if (confirmCompareBlockIsBelongableAtSockets(block,
	 * compareBlockParent)) { return true; } }
	 * }//abstructionブロック内のすべてのブロックを探索し終えた } return false; }
	 */
	// 親のブロックを探索する
	...
	// }
	//
	// while (Block.getBlock(checkRBlock.getBlock().getBeforeBlockID()) != null)
	// {//直前ブロックがnullになるまで
	...
	public static boolean isCompareBlock(Block block) {
		if (block.getGenusName().equals("gettersuper") || block.getGenusName().equals("getterthis")) {
			return false;
		}
		if (block.getGenusName().startsWith("getter") || block.getGenusName().startsWith("setter")
	...
	public static boolean isAloneBlock(Block block) {
		if (isIndependentBlock(block) && Block.getBlock(block.getAfterBlockID()) == null) {
			return true;
		}
		return false;
	}
