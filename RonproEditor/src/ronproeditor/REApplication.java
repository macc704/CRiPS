...
public class REApplication implements ICFwApplication {
	...
	public void doOpen(File file) {
		...
		if (file.getName().endsWith("java") || file.getName().endsWith("js")) {// @TODO きちんと実装すること 2011/11/22
	...
	public void doSave() {
		if (getSourceManager().hasCurrentFile()) {
			...
			blockManager.doRefleshBlock(getSourceManager().getCurrentFile()); // 要：ファイル削除の前に実行
