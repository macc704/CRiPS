...
public class BlockStub extends Block {
	 ...
	 * connection is more apparent when the parent Block is removed/deleted. The
	 * stubs become dangling references. These stubs are resolved when a new
	 * parent block is created with the previous parent's name.
	 * */
	...
	/**
	 * Temporary mapping for parent type (caller plugs). remove once
	 * BlockUtilities cloneBlock() is finished
	 */
	...
	// this particular stub type is unique to Starlogo TNG - may choose to
	// remove it
	...
	/**
	 * mySocketToParentSocket maps the sockets of this stubs to the sockets of
	 * its parent this mapping is used to help in the maintanence this stub's
	 * sockets with respect to its parent
	 */
	 ...
	 * genus name of its parent block, the block id of its parent, the block
	 ...
	 * @param parentGenus
	 *            the BlockGenus String name of its initial parent
	 * @param parentName
	 * @param stubGenus
	 */
	public BlockStub(Long initParentID, String parentGenus, String parentName,
			String stubGenus) {
		super(stubGenus);
		...
		// initialize stub properties based on stubGenus such as sockets, plugs,
		// and labels
		// this initialization assumes that nothing is connected to the parent
		// yet
		// note: instead of modifying the stub blocks currect sockets, we
		// replace them with whole new ones
		// such that the initkind of the stub blocks connectors are the same as
		// their parents
		if (stubGenus.startsWith(GETTER_STUB)) {
			...
			// sockets)
			if (parent.getSocketAt(0).getBlockID() != Block.NULL
					|| parent.getNumSockets() > 1) {
			...
			// remove the following once BlockUtilities.cloneBlock() is
			// finished
			// If our parent already has a plug type, we want to update
			// Note that we don't need to call renderables, since we are still
			// in the constructor
			String kind = parentToPlugType.get(parent.getBlockLabel()
					+ parent.getGenusName());
			if (kind != null) {
				removeBeforeAndAfter();
				// ria commented code relates to creating mirror plugs for
			...
			// set plug to be parent single socket kind or plug kind if parent
			// has no sockets
			if (parent.getNumSockets() > 0)
				setPlug(parent.getSocketAt(0).getKind(), this.getPlug()
						.getPositionType(), this.getPlugLabel(), this.getPlug()
						.isLabelEditable(), this.getPlugBlockID());
			else
				setPlug(parent.getPlugKind(), this.getPlug().getPositionType(),
						this.getPlugLabel(), this.getPlug().isLabelEditable(),
						this.getPlugBlockID());
		...
		parentNameToBlockStubs.get(parentName + parentGenus).add(
				this.getBlockID());
	 ...
	 * @param stubGenus
	 *            the BlockGenus of this
	 * @param label
	 *            the Block label of this
	 * @param parentName
	 *            the String name of its parent
	 * @param parentGenus
	 *            the String BlockGenus name of its parent
	 */
	protected BlockStub(Long blockID, String stubGenus, String label,
			String parentName, String parentGenus) {
		...
		// there's a chance that the parent for this has not been added to
		// parentNameToBlockStubs mapping
	...
	public static Iterable<Long> getStubsOfParent(Long blockID) {
		ArrayList<Long> stubs = parentNameToBlockStubs.get(Block.getBlock(
				blockID).getBlockLabel()
	...
	public static void putNewParentInStubMap(Long blockID) {
		String key = Block.getBlock(blockID).getBlockLabel()
				+ Block.getBlock(blockID).getGenusName();
	 ...
	 * @param parentID
	 */
	public static void parentNameChanged(String oldParentName,
			String newParentName, Long parentID) {
		...
		// only update if parents name really did "change" meaning the new
		// parent name is
	...
	public boolean doesParentExist() {
		// ria: needs to check BlockCanvas if parent is "alive"
	...
	/**
	 * Updates the conenctors of this stub according to its parent. For now only
	 * caller stubs should update their connector information after being
	 * created.
	 */
	private void updateConnectors() {
		...
		if (parent != null) {
			// retrieve sockets from parent and set sockets accordingly
			Iterator<BlockConnector> parentSockets = parent.getSockets()
					.iterator();
			int i; // socket index
			...
			for (i = 0; parentSockets.hasNext(); i++) {
				...
				if (parentSocket.getBlockID() != Block.NULL) {
					...
					if (i > this.getNumSockets() - 1) {
						// socket labels should correspond with the socket
						// blocks of parent
						if (parentSocket.getBlockID() != Block.NULL)
							addSocket(parentSocket.getKind(),
			...
			// caller stubs that have no sockets
			/*
			 * if(stubGenus.startsWith(CALLER_STUB)){ if(this.getNumSockets() >
			 * 0) { if(this.hasPlug() &&
			 * this.getPlug().getPositionType().equals(
			 * BlockConnector.PositionType.MIRROR)) {
			 * this.getPlug().setPositionType(PositionType.SINGLE); } } else {
			 * if(this.hasPlug() &&
			 * this.getPlug().getPositionType().equals(BlockConnector
			 * .PositionType.SINGLE)) {
	...
	private void updatePlug(String kind) {
		...
		// Always synchronize! We can't have both a plug and a before.
		removeBeforeAndAfter();
		// ria commented code relates to creating mirror plugs for caller
		// stubs that have no sockets
		/*
		 * if (this.getNumSockets() == 0) { setPlug(kind, PositionType.MIRROR,
		 * kind, false, Block.NULL); } else {
		 */
	...
	private void disconnectBlock(Long id) {
		...
		RenderableBlock rb = RenderableBlock.getRenderableBlock(link
				.getSocketBlockID());
		link.disconnect();
		rb.blockDisconnected(link.getSocket());
		Workspace.getInstance().notifyListeners(
				new WorkspaceEvent(rb.getParentWidget(), link,
