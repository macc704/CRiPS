 ...
 * This includes the different (command/data) (sockets/plugs). The standard
 * sockets/plugs have varying styles.
 * 
 * BlockConnectionShape doesn't care about the layout of the connectors on the
 * blocks, that is the job of BlockShape.
 * 
...
public class BlockConnectorShape {
	...
	/**
	 * created by sakai lab 2011/11/20 Width of arrow plug shape
	 */
	public static final float ARROW_DATA_PLUG_WIDTH = 16.0f;
	...
	/** height of abstraction block-socket */
	public static final float ABSTRACTION_BLOCK_SOCKET_HEIGHT = 5f;

	/** width of command input bar */
	public static final float COMMAND_INPUT_BAR_WIDTH = BlockShape.COMMAND_PORT_OFFSET + 2f;

	/** width of abstaction input bar */
	public static final float ABSTRACTION_LEFT_BAR_WIDTH = 5f;

	/** height of command input bar */
	public static final float COMMAND_INPUT_BAR_HEIGHT = 5f;
	/** height of abstraction block input bar */
	public static final float ABSTRACTION_INPUT_BAR_HEIGHT = 28f;
	/** default height of command input */
	public static final float DEFAULT_COMMAND_INPUT_HEIGHT = DATA_PLUG_HEIGHT;
	/** default height of abstract input */
	public static final float DEFAULT_ABSTRACT_INPUT_HEIGHT = DATA_PLUG_HEIGHT
			+ ABSTRACTION_BLOCK_SOCKET_HEIGHT;
	...
	/**
	 * A hashmap mapping the application specific string of connection shapes to
	 * a shape identifying integer
	 */
	...
	/**
	 * Different styles of SocketShapes: 1 is the normal shape 2 is the double
	 * stacked shape 3 is the double inversion
	 */
	...
	public static final int TRAPEZOID = 16;
	...
	private void addDebugConnectionShapeMappings() {
		...
		addConnenctionShapeMapping("object", 15);
	}
	...
	/**
	 * Add a mapping of an application specific string of a connection shape to
	 * a shape identifying integer
	 */
	...
	/**
	 * Get a shape identifying integer mapped to an application specific string
	 * of a connection shape
	 */
	...
	/**
	 * Gets the dimension of a given BlockConnector. Mapping for the connector
	 * to a shape must already exist.
	 */
	public static Dimension getConnectorDimensions(BlockConnector blockConnector) {
		...
		assert (mappedValue != -1) : "Block Connector is not mapped: "
				+ blockConnector;

		// add proc param dimensions
		if (mappedValue == POLYMORPHIC_1 || mappedValue == POLYMORPHIC_2
				|| mappedValue == POLYMORPHIC_3) {
			return new Dimension((int) POLYMORPHIC_DATA_PLUG_WIDTH,
					(int) DATA_PLUG_HEIGHT);
		}
		// arranged by sakai lab 2011/11/20
		else if (mappedValue == ARROW) {
			return new Dimension((int) ARROW_DATA_PLUG_WIDTH,
					(int) DATA_PLUG_HEIGHT);
		} else {
			return new Dimension((int) NORMAL_DATA_PLUG_WIDTH,
					(int) DATA_PLUG_HEIGHT);
		}
	}

	/**
	 * created by sakai lab 2011/11/20 Gets data plug width of a given
	 * BlockConnector. Mapping for the connector to a shape must already exist.
	 * 
	 * @param blockConnector
	 * @return data plug width of block
	 */
	public static int getDataPlugWidth(BlockConnector blockConnector) {
		int mappedValue = getConnenctionShapeMapping(blockConnector.getKind());

		// if shaped not yet mapped
		assert (mappedValue != -1) : "Block Connector is mapped:"
				+ blockConnector;

		// return data plug width
		if (mappedValue == POLYMORPHIC_1 || mappedValue == POLYMORPHIC_2
				|| mappedValue == POLYMORPHIC_3) {
			return (int) POLYMORPHIC_DATA_PLUG_WIDTH;
		} else if (mappedValue == ARROW) {
			return (int) ARROW_DATA_PLUG_WIDTH;
		}
		return (int) NORMAL_DATA_PLUG_WIDTH;
	...
	// //////////////////////////////////
	public Point2D addControlConnectorShape(GeneralPath blockPath,
			boolean appendRight) {

		return addControlConnectorShape(blockPath, CONTROL_PLUG_WIDTH / 2,
				appendRight);

	}

	public Point2D addControlConnectorShape(GeneralPath blockPath,
			float distanceToCenter, boolean appendRight) {
		...
		Point2D socketPoint = new Point2D.Float((float) startPoint.getX()
				+ (appendRight ? distanceToCenter : -distanceToCenter),
	...
	public Point2D addCommandSocket(GeneralPath blockPath,
		...
		BlockShapeUtil.cornerTo(blockPath, new Point2D.Float((float) blockPath
				.getCurrentPoint().getX()
				- COMMAND_INPUT_BAR_WIDTH
				+ BlockShape.CORNER_RADIUS, (float) blockPath.getCurrentPoint()
				.getY()), new Point2D.Float((float) blockPath.getCurrentPoint()
				.getX() - COMMAND_INPUT_BAR_WIDTH + BlockShape.CORNER_RADIUS,
				(float) blockPath.getCurrentPoint().getY()
						+ BlockShape.CORNER_RADIUS), BlockShape.CORNER_RADIUS);
		...
		BlockShapeUtil.cornerTo(blockPath, new Point2D.Float((float) blockPath
				.getCurrentPoint().getX(), (float) blockPath.getCurrentPoint()
				.getY() + BlockShape.CORNER_RADIUS), new Point2D.Float(
				(float) blockPath.getCurrentPoint().getX()
						+ BlockShape.CORNER_RADIUS, (float) blockPath
						.getCurrentPoint().getY() + BlockShape.CORNER_RADIUS),
	...
	public Point2D addCommandSocket(GeneralPath blockPath,
			int commandSocketHeight, BlockConnector curSocket) {
	...
	// ------------------------------------------------
	// arrenged by sakai lab 2010/12
	// ABSTRACTION BLOCK SOCKET (OVERHANG AND AREA)
	// ------------------------------------------------

	public Point2D addAbstractionBlockSocket(GeneralPath blockPath,
			int commandSocketHeight) {

		// draw bar
		BlockShapeUtil.lineToRelative(blockPath, 0,
				ABSTRACTION_INPUT_BAR_HEIGHT);
		float xLast = (float) blockPath.getCurrentPoint().getX()
				- (ABSTRACTION_LEFT_BAR_WIDTH + BlockShape.CORNER_RADIUS * 2);
		BlockShapeUtil.lineToRelative(blockPath, (float) -blockPath
				.getCurrentPoint().getX()
				+ CONTROL_PLUG_WIDTH
				+ ABSTRACTION_LEFT_BAR_WIDTH
				+ BlockShape.CORNER_RADIUS
				+ BlockShape.COMMAND_PORT_OFFSET, 0);
		Point2D socketPoint = addControlConnectorShape(blockPath, false);

		// first corner inside command input

		BlockShapeUtil.cornerTo(blockPath, new Point2D.Float(
				ABSTRACTION_LEFT_BAR_WIDTH + BlockShape.CORNER_RADIUS,
				(float) blockPath.getCurrentPoint().getY()), new Point2D.Float(
				ABSTRACTION_LEFT_BAR_WIDTH + BlockShape.CORNER_RADIUS,
				(float) blockPath.getCurrentPoint().getY()
						+ BlockShape.CORNER_RADIUS), BlockShape.CORNER_RADIUS);
		...
		// insert dynamic command input height between these two methods
		BlockShapeUtil.lineToRelative(blockPath, 0, commandSocketHeight
				- SAbstractionBlockShape.ABSTRACTION_SOCKET_UPPER_SPACE);

		xLast += BlockShape.CORNER_RADIUS;
		// System.out.println("fourth:" + blockPath.getCurrentPoint());
		// second corner at bottom of command input
		BlockShapeUtil.cornerTo(blockPath, new Point2D.Float((float) blockPath
				.getCurrentPoint().getX(), (float) blockPath.getCurrentPoint()
				.getY() + BlockShape.CORNER_RADIUS), new Point2D.Float(
		...
		xLast -= blockPath.getCurrentPoint().getX();

		if (commandSocketHeight != BlockConnectorShape.DEFAULT_COMMAND_INPUT_HEIGHT) {
			BlockShapeUtil.lineToRelative(blockPath, ABSTRACTION_LEFT_BAR_WIDTH
					+ BlockShape.CORNER_RADIUS * 2, 0);
			addControlConnectorShape(blockPath, CONTROL_PLUG_WIDTH / 2, true);

			xLast -= ABSTRACTION_LEFT_BAR_WIDTH + BlockShape.CORNER_RADIUS * 2
					+ CONTROL_PLUG_WIDTH - 0.5f;
		}

		// extend left to match y coordinate of initial point
		BlockShapeUtil.lineToRelative(blockPath,
				xLast + CONTROL_PLUG_WIDTH / 2, 0);
	 ...
	 * 
	 * @param blockPath
	 *            the current GeneralPath of the block being drawn
	 * @param connectionShape
	 *            specifies the socket shape to be added
	 * @param onRightSide
	 *            (note this method assumes startingFromTop)
	 */
	public Point2D addDataSocket(GeneralPath blockPath, String connectionShape,
			boolean onRightSide) {
	...
	public Point2D addDataSocketUp(GeneralPath blockPath,
			String connectionShape, boolean onRightSide) {
		// if onRightSide, socket is convex-left
		return addDataConnection(blockPath, connectionShape, false,
				!onRightSide);
	}
	 ...
	 *            (note this method assumes startingFromTop)
	 */
	public Point2D addDataPlug(GeneralPath blockPath, String connectionShape,
			boolean onRightSide) {
	...
	// assosiated method to draw starting from the bottom of the socket
	public Point2D addDataPlugUp(GeneralPath blockPath, String connectionShape,
			boolean onRightSide) {
		// if onRightSide, plug is convex-right
	...
	/**
	 * Appends a specific data connection GeneralPath to a given GeneralPath.
	 * startFromTop specifies that the end of the current generalPath is at the
	 * top of the data connection to be drawn. convexRight means that the curve
	 * drawn points to the right.
	 * 
	 * The use of _lineTo and _curveTo are to reduce code clutter and to make it
	 * clear that these are private methods and not the GeneralPath.lineTo or
	 * GeneralPath.curveTo. _lineTo and _curveTo are relative to the starting
	 * point of the the data connection to be drawn.
	 * 
	 * @param blockPath
	 *            is the current GeneralPath to be appended
	 * @param socketShape
	 *            is the integer representing the kind and style of socket to be
	 *            drawn
	 * @param startFromTop
	 *            true if end of the current generalPath is at the top of the
	 *            data connection to be drawn
	 * @param convexRight
	 *            true if curve drawn points to the right
	 */
	private Point2D addDataConnection(GeneralPath blockPath,
			String connectionShape, boolean startFromTop, boolean convexRight) {
		...
		Point2D socketPoint = new Point2D.Float(
				(float) startPoint.getX(),
				(startFromTop ? ((float) startPoint.getY() + ((int) DATA_PLUG_HEIGHT / 2))
						: ((float) startPoint.getY() - ((int) DATA_PLUG_HEIGHT / 2))));
		...
		// Starlogo Boolean
		case CIRCLE_1:
			_curveTo((NORMAL_DATA_PLUG_WIDTH) * 4 / 3, 0,
					(NORMAL_DATA_PLUG_WIDTH) * 4 / 3, DATA_PLUG_HEIGHT, 0,
					DATA_PLUG_HEIGHT);
			break;

		case CIRCLE_2:
			_curveTo(NORMAL_DATA_PLUG_WIDTH, 0, NORMAL_DATA_PLUG_WIDTH,
					DATA_PLUG_HEIGHT * 1 / 4, NORMAL_DATA_PLUG_WIDTH * 1 / 2,
					DATA_PLUG_HEIGHT * 1 / 2);
			_curveTo(NORMAL_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT * 3 / 4,
					NORMAL_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT, 0,
					DATA_PLUG_HEIGHT);
			break;
		...
		case POLYMORPHIC_1:
			_curveTo(0, DATA_PLUG_HEIGHT / 3, POLYMORPHIC_DATA_PLUG_WIDTH / 3,
					DATA_PLUG_HEIGHT / 3, POLYMORPHIC_DATA_PLUG_WIDTH / 2,
					DATA_PLUG_HEIGHT / 4);
			...
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT * 5 / 6,
					POLYMORPHIC_DATA_PLUG_WIDTH * 2 / 3,
					DATA_PLUG_HEIGHT * 5 / 6, POLYMORPHIC_DATA_PLUG_WIDTH / 2,
					DATA_PLUG_HEIGHT * 3 / 4);
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH / 3, DATA_PLUG_HEIGHT * 2 / 3,
					0, DATA_PLUG_HEIGHT * 2 / 3, 0, DATA_PLUG_HEIGHT);
			break;

		case POLYMORPHIC_2:
			_curveTo(0, DATA_PLUG_HEIGHT / 6, POLYMORPHIC_DATA_PLUG_WIDTH / 3,
					DATA_PLUG_HEIGHT / 6, POLYMORPHIC_DATA_PLUG_WIDTH / 2,
					DATA_PLUG_HEIGHT / 8);
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH * 2 / 3,
					DATA_PLUG_HEIGHT / 12, POLYMORPHIC_DATA_PLUG_WIDTH,
					DATA_PLUG_HEIGHT / 12, POLYMORPHIC_DATA_PLUG_WIDTH,
					DATA_PLUG_HEIGHT / 4);
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT * 5 / 12,
					POLYMORPHIC_DATA_PLUG_WIDTH * 2 / 3,
					DATA_PLUG_HEIGHT * 5 / 12, POLYMORPHIC_DATA_PLUG_WIDTH / 2,
					DATA_PLUG_HEIGHT * 3 / 8);
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH / 3, DATA_PLUG_HEIGHT * 2 / 6,
					0, DATA_PLUG_HEIGHT * 2 / 6, 0, DATA_PLUG_HEIGHT / 2);
			// shifted duplicate
		...
		case POLYMORPHIC_3:
			...
			_curveTo(POLYMORPHIC_DATA_PLUG_WIDTH * 10 / 9,
					DATA_PLUG_HEIGHT * 0.15f,
					POLYMORPHIC_DATA_PLUG_WIDTH * 10 / 9,
					DATA_PLUG_HEIGHT * 0.35f,
					POLYMORPHIC_DATA_PLUG_WIDTH * 3 / 4,
					DATA_PLUG_HEIGHT * 3 / 8);
			...
			_lineTo(-POLYMORPHIC_DATA_PLUG_WIDTH * 3 / 4, DATA_PLUG_HEIGHT / 8
					+ DATA_PLUG_HEIGHT / 2);
			_curveTo(-POLYMORPHIC_DATA_PLUG_WIDTH * 10 / 9, DATA_PLUG_HEIGHT
					* 0.15f + DATA_PLUG_HEIGHT / 2,
					-POLYMORPHIC_DATA_PLUG_WIDTH * 10 / 9, DATA_PLUG_HEIGHT
							* 0.35f + DATA_PLUG_HEIGHT / 2,
					-POLYMORPHIC_DATA_PLUG_WIDTH * 3 / 4, DATA_PLUG_HEIGHT * 3
							/ 8 + DATA_PLUG_HEIGHT / 2);
			_lineTo(-POLYMORPHIC_DATA_PLUG_WIDTH / 2, DATA_PLUG_HEIGHT * 0.475f
					+ DATA_PLUG_HEIGHT / 2);
			_lineTo(-POLYMORPHIC_DATA_PLUG_WIDTH / 3, DATA_PLUG_HEIGHT * 3 / 8
					+ DATA_PLUG_HEIGHT / 2);
		...
		// TODO ここでconnectorの形を変える
		// created by sakai lab 2011/11/19
		// original shape by blockEditor
		case ARROW:
			_lineTo(0, DATA_PLUG_HEIGHT * 0.3f);
			_lineTo(ARROW_DATA_PLUG_WIDTH / 2, DATA_PLUG_HEIGHT * 0.3f);
			_lineTo(ARROW_DATA_PLUG_WIDTH / 2, DATA_PLUG_HEIGHT * 0.2f);
			_lineTo(ARROW_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT / 2);
			_lineTo(ARROW_DATA_PLUG_WIDTH / 2, DATA_PLUG_HEIGHT * 0.8f);
			_lineTo(ARROW_DATA_PLUG_WIDTH / 2, DATA_PLUG_HEIGHT * 0.7f);
			_lineTo(0, DATA_PLUG_HEIGHT * 0.7f);
			_lineTo(0, DATA_PLUG_HEIGHT);
			break;
		case TRAPEZOID:
			_lineTo(NORMAL_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT * 0.25f);
			_lineTo(NORMAL_DATA_PLUG_WIDTH, DATA_PLUG_HEIGHT * 0.75f);
			_lineTo(0, DATA_PLUG_HEIGHT);
			break;
		...
		if (!startFromTop || !convexRight) {
			transformGeneralPath(currentConnectorPath, !convexRight,
					!startFromTop);
		}
	...
	private void _lineTo(float x, float y) {
		currentConnectorPath.lineTo(x + (float) startPoint.getX(), y
				+ (float) startPoint.getY());
	}
	 ...
	 * drawing a B�ｽzier curve that intersects both the current coordinates and
	 * the coordinates (x3, y3), using the specified points (x1, y1) and (x2,
	 * y2) as B�ｽzier control points.
	 */
	private void _curveTo(float x1, float y1, float x2, float y2, float x3,
			float y3) {
		currentConnectorPath.curveTo(x1 + (float) startPoint.getX(), y1
				+ (float) startPoint.getY(), x2 + (float) startPoint.getX(), y2
				+ (float) startPoint.getY(), x3 + (float) startPoint.getX(), y3
				+ (float) startPoint.getY());
	}

	/**
	 * Flips a GeneralPath and translates it so the starting point is in the
	 * correct place.
	 * 
	 * @param gp
	 *            the GeneralPath to be transformed
	 * @param horzFlip
	 *            true if flipped horizontally
	 * @param vertFlip
	 *            true if flipped vertically
	 */
	private void transformGeneralPath(GeneralPath gp, boolean horzFlip,
			boolean vertFlip) {
		int xScale, yScale;
	 ...
	 * 
	 * @param root
	 *            the Element carrying the specifications of the
	...
	public static void loadBlockConnectorShapes(Element root) {
		...
		for (int i = 0; i < drawerNodes.getLength(); i++) {
			...
			if (drawerNode.getNodeName().equals("BlockConnectorShape")) {
				...
				nameMatcher = attrExtractor.matcher(drawerNode.getAttributes()
						.getNamedItem("shape-type").toString());
				if (nameMatcher.find()) // will be true
					shapeType = nameMatcher.group(1);
				nameMatcher = attrExtractor.matcher(drawerNode.getAttributes()
						.getNamedItem("shape-number").toString());
				if (nameMatcher.find()) // will be true
				...
				if (shapeType != null && shapeNumber != null) {
					...
					BlockConnectorShape.addConnenctionShapeMapping(shapeType,
							Integer.parseInt(shapeNumber));
				}
			}
		}
	...
	static public boolean isCommandConnector(BlockConnector connector) {
		return (BlockConnectorShape.getConnenctionShapeMapping(connector
				.getKind()) == BlockConnectorShape.COMMAND);
	}
