...
public class RESourceEditor {
	...
	public void refresh() {
		if (isDirty()) {
			JOptionPane.showMessageDialog(application.getFrame(), "不正な更新",
					"編集されています", JOptionPane.ERROR_MESSAGE);
			return;
		}
		setText(loadText());
	}
	...
	class MyDocumentListener implements DocumentListener, UndoableEditListener {
		...
		public void changedUpdate(DocumentEvent e) {
			...
			// setDirty(true); // fontを変えたときにdirtyになってしまう．
		}
	...
	public void format() {
		...
		if (!text.equals(newText)) {
			...
			// getViewer().getTextPane().getDocument().addDocumentListener(documentListener);//2012.10.10危険なので一旦ヤメ
			setText(newText);
			// getViewer().getTextPane().getDocument().addDocumentListener(documentListener);//2012.10.10危険なので一旦ヤメ
