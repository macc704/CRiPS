...
/**
 * FactoryRenderableBlock extends RenderableBlock and is used within
 * FactoryBlockDrawers. Unlike its superclass RenderableBlock,
 * FactoryRenderableBlock does not move or connect to any blocks. Instead it has
 * one function only, to produce new RenderableBlocks and their associated Block
 * instances. It's block labels are also uneditable.
 * 
 * When a mouse is pressed over a FactoryRenderableBlock, a new RenderableBlock
 * instance is created on top of it to receive further mouse events and a new
 * Block instance is created in the background.
 */
public class FactoryRenderableBlock extends RenderableBlock {
	...
	// we have this instance of the dragHandler so that we can use it mouse
	// entered
	// mouseexited methods to change the cursor appropriately, so that we can
	// make it
	// "seem" that this block is draggable
	private JComponentDragHandler dragHandler;

	/**
	 * TODO 変数定義ブロックについて、 FactoryではnameLabelを表示しない
	 * 
	 * Constructs a new FactoryRenderableBlock instance.
	 * 
	 * @param widget
	 *            the parent widget of this
	 * @param blockID
	 *            the Long ID of its associated Block instance
	 */
	public FactoryRenderableBlock(WorkspaceWidget widget, Long blockID) {
		...
		// arrenged by sakai lab 2011/11/18
	...
	/**
	 * Returns a new RenderableBlock instance (and creates its associated Block)
	 * instance of the same genus as this.
	 * 
	 * @return a new RenderableBlock instance with a new associated Block
	 *         instance of the same genus as this.
	 */
	...
	public void mousePressed(MouseEvent e) {
		...
		// createdRB not really "added" to widget (not necessary to since it
		// will be removed)
		...
		mouseDragged(e); // immediately make the RB appear under the mouse
							// cursor
	}
