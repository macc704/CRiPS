...
public class CTracklessScrollPane extends CScrollPane implements KeyListener{
	 ...
	 * This is duplicate code (In Hover, Glass, and Tackless ScollPanes)
 ...
 * the PARALLEL bouding b2j.model range passed as an argument in the
 * constructor.  Any changes to this scrollbar's thumb position
 * should perform the same parallel changes to the bounding b2j.model range.
 * 
 * @author An Ho
 *
 */
class VerticalBar extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener{
	...
	/**
	 * Bounding b2j.model that this vertical scroll bar will change when the thumb
	 * is moved.  This bar also collects information from the bounding
	 * b2j.model (including the value, maximum, minimum, and extent) in order
	 ...
	 * 
	 * @requires view != null
	 * @return a tranformed value from view coordinates to b2j.model coordinates
	 */
	private double viewToModel(int view){
		return view*modelrange.getMaximum()/this.getHeight();
	}
	/**
	 * @param b2j.model - b2j.model value to tranform
	 * 
	 * @requires b2j.model != null
	 * @return a tranformed value from b2j.model coordinates to view coordinates
	 */
...
class HorizontalBar extends JPanel implements MouseListener, MouseMotionListener{
	...
	/**
	 * Bounding b2j.model that this horizontal scroll bar will change when the thumb
	 * is moved.  This bar also collects information from the bounding
	 * b2j.model (including the value, maximum, minimum, and extent) in order
	 * to render the corect size and location of the thumb.
	 */
