...
public class InfixRule implements LinkRule, WorkspaceListener {
	 ...
	 * @param socket1
	 *            a <code>Socket</code> of block1
	 * @param socket2
	 *            a <code>Socket</code> of block2
	...
	public void workspaceEventOccurred(WorkspaceEvent e) {
		if (e.getEventType() == WorkspaceEvent.BLOCKS_CONNECTED) {
			...
			if (!newchild.hasPlug())
				return;
			Block newChildPlug = Block
					.getBlock(newchild.getPlug().getBlockID());
			...
			if (newChildSocket.hasBlock())
				return;
			BlockLink link2 = BlockLink.getBlockLink(newchild, oldchild,
					newchild.getSocketAt(0), oldchild.getPlug());
