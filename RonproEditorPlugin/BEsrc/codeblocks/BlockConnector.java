 ...
 * information for each socket or plug of a particular Block. Each socket has a
 * kind (i.e. number, String, boolean, etc.), a label, and the block id of the
 * block at that socket (not to be confused with the block that hold the socket
 * information - socket does not have a reference to that parent block).
 */
public class BlockConnector implements ISupportMemento {

	// need some sort of indication if there is a block in this socket, i.e. -1
	...
	private String connectorJavaType = null;

	// Specifies the PositionType of connector:
	// Single is the default connector that appears on only one side
	// (left/right) of a block.
	// Mirror is creates a connectors with locations mirrored on both left and
	// right side of a block.
	// Bottom is a double sided enclosure within the bottom side of a block, or
	// the next command.
	// Top is the previous command.
	public enum PositionType {
		SINGLE, MIRROR, BOTTOM, TOP
	};
	 ...
	 *
	 * @param kind
	 *            the kind of this socket
	 * @param positionType
	 *            the PositionType of connector
	 * @param label
	 *            the String label of this socket
	 * @param isLabelEditable
	 *            is true iff this BlockConnector can have its labels edited.
	 * @param isExpandable
	 *            whether this socket can expand into another connector when a
	 *            block is connected
	 * @param expandGroup
	 *            the expand socket group of this connector
	 * @param connBlockID
	 *            the ID of the block connected to this
	 */
	public BlockConnector(String kind, PositionType positionType, String label,
			boolean isLabelEditable, boolean isExpandable, String expandGroup,
			Long connBlockID) {
		this(kind, positionType, label, isLabelEditable, isExpandable,
				connBlockID);
	 ...
	 *            is true iff this BlockConnector can have its labels edited.
	 * @param isExpandable
	 *            true if this socket can expand into another connector when a
	 *            block is connected to this
	 * @param positionType
	 *            specifies the PositionType of connector
	 */
	public BlockConnector(String kind, PositionType positionType, String label,
			boolean isLabelEditable, boolean isExpandable, Long connBlockID) {
		this.kind = kind;
	 ...
	 *            the kind of this socket
	 * @param socketBlockID
	 *            the block id attached to this socket
	 */
	...
	/**
	 * Constructs a new <code>BlockConnector</code> with the specified label and
	 * kind. This new socket does not have an attached block.
	 *
	 * @param label
	 *            the String label of this socket
	 * @param isLabelEditable
	 ...
	 *            the kind of this socket
	 */
	public BlockConnector(String label, String kind, boolean isLabelEditable,
			boolean isExpandable) {
		this(kind, PositionType.SINGLE, label, isLabelEditable, isExpandable,
				Block.NULL);
	}

	/**
	 * Constucts a new <code>BlockConnector</code> by copying the connector
	 * information from the specified con. Copies the con's connector label and
	 * kind.
	 *
	 * @param con
	 *            the BlockConnector to copy from
	 */
	public BlockConnector(BlockConnector con) {
		...
		this.connectorJavaType = con.getConnectorJavaType();
	...
	public void setCnnectorJavaType(String type){
		this.connectorJavaType = type;
	}

	public String getConnectorJavaType(){
		return this.connectorJavaType;
	}
	...
	/**
	 * Returns true iff this connector is expandable, meaning if a block is
	 * connected to it, it may cause another empty connector just like this to
	 * appear. Whether or not a block actually appears depends on this
	 * connector's parent block. Technically only sockets can expand. Each block
	 ...
	 *
	 * @param id
	 *            the block id of the desired block to attach
	 */
	 ...
	 *
	 * @param pos
	 *            the desired PositionType for this
	 */
	...
	/**
	 * Sets this connector's default argument to the specified genus and initial
	 * label.
	 *
	 * @param genusName
	 *            the desired BLockGenus name of the default agrument
	 * @param label
	 *            the initial label of the default argument
	 */
	 ...
	 * returns the block ID of the connected default argument or Block.NULL if
	 * there is none.
	 *
	 * @return the block ID of the connected default argument or Block.NULL if
	 *         there is none.
	 */
	...
	/**
	 * <code>DefArgument</code> is a lightweight class that stores Default
	 * Argument information of this connector if it has any, particular the name
	 * of the argument's genus and its initial label. Each connector has at most
	 * 1 default argument.
	 */
	...
	// //////////////////////
	/**
	 * Loads information for a single BlockConnector and returns an instance of
	 * BlockConnector with the loaded information
	 *
	 * @param node
	...
	public static BlockConnector loadBlockConnector(Node node,
			HashMap<Long, Long> idMapping, String genusLabel) {
		...
		if (node.getNodeName().equals("BlockConnector")) {
			...
			nameMatcher = attrExtractor.matcher(node.getAttributes()
					.getNamedItem("init-type").toString());
			if (nameMatcher.find()) // will be true
				initKind = nameMatcher.group(1);
			nameMatcher = attrExtractor.matcher(node.getAttributes()
					.getNamedItem("connector-type").toString());
			if (nameMatcher.find()) // will be true
				kind = nameMatcher.group(1);

			//#matsuzawa 2012.11.11 何をしているのか分からないけど，とりあえず削ってみる．
			...
			if (opt_item != null) {
				...
				if (nameMatcher.find()) // will be true
					idConnected = Block.translateLong(
			...
			if (positionType.equals("single"))
				con = new BlockConnector(initKind, PositionType.SINGLE, label,
						isLabelEditable, isExpandable, idConnected);
			else if (positionType.equals("bottom"))
				con = new BlockConnector(initKind, PositionType.BOTTOM, label,
						isLabelEditable, isExpandable, idConnected);
			else if (positionType.equals("mirror"))
				con = new BlockConnector(initKind, PositionType.MIRROR, label,
						isLabelEditable, isExpandable, idConnected);
			else if (positionType.endsWith("top"))
				con = new BlockConnector(initKind, PositionType.TOP, label,
						isLabelEditable, isExpandable, idConnected);
	...
	/**
	 * Returns the save string of this. save string only includes information
	 * that was modifiable and modified
	 *
	 * @param conKind
	...
	public String getSaveString(String conKind) {
		...
		if (!this.connBlockID.equals(Block.NULL))
			appendAttribute("con-block-id", this.connBlockID.toString(),
					saveString);
	...
	public void loadState(Object memento) {
		if (memento instanceof BlockConnectorState) {
			...
			if (state.hasDefArg) {
				this.setDefaultArgument(state.defArgGenusName,
						state.defArgLabel);
			} else {
				this.arg = null;
			}
	...
	public static BlockConnector instantiateFromState(Object memento) {
		if (memento instanceof BlockConnectorState) {
			...
			BlockConnector instance = new BlockConnector(state.kind,
					state.positionType, state.label, state.isLabelEditable,
					state.isExpandable, state.connBlockID);
			instance.isLabelEditable = state.isLabelEditable;

			if (state.hasDefArg) {
				instance.setDefaultArgument(state.defArgGenusName,
						state.defArgLabel);
			}
			return instance;
		}
