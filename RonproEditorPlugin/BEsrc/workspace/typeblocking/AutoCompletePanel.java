...
/**
 * AutCompletePanel is a Panel that displays an editable text field for the user
 * to produce a desired pattern (regex). It also provides a JList for users to
 * see some pre-defined choices. User may choose to act/react to
 * AutoCompletePanel intuitively.
 */
@SuppressWarnings("rawtypes")
public class AutoCompletePanel extends JPanel implements MouseListener,
		MouseMotionListener {
	...
	public AutoCompletePanel() {
		...
		CTracklessScrollPane menuPane = new CTracklessScrollPane(menu, 7,
				new Color(75, 50, 0), BACKGROUND) {
	...
	public void mousePressed(MouseEvent e) {
		if (e.getX() > (this.getWidth() - 2 * MARGIN)
				&& e.getY() > (this.getHeight() - 2 * MARGIN)) {
			resizing = true;
		}
	}
	...
	public void mouseDragged(MouseEvent e) {
		if (resizing) {
			preferredWidth = e.getX() > MINIMUM_WIDTH ? e.getX()
					: MINIMUM_WIDTH;
			preferredHeight = e.getY() > MINIMUM_HEIGHT ? e.getY()
					: MINIMUM_HEIGHT;
			updateMenu();
		}
	}
	...
	/**
	 * Updates the menu such that it can display all the possible blocks that
	 * match the user-generated pattern (the user-generated pattern is entered
	 * within AutoCompletePanel.editor)
	 */
		...
		if (obj != null && obj instanceof TextualFactoryBlock) {
			...
			try {
				...
				TypeBlockManager.automateBlockInsertion(
						(TextualFactoryBlock) obj, obj.toString());
			} catch (NumberFormatException e) {
				// if "+" then pass the two labels in
				if (obj.toString().equals(
						TypeBlockManager.NUMBER_PLUS_OPERATION_LABEL)
						|| obj.toString().equals(
								TypeBlockManager.TEXT_PLUS_OPERATION_LABEL)) {
					TypeBlockManager.automateBlockInsertion(
							(TextualFactoryBlock) obj, obj.toString());
					// if starts with quote (is a string block)
				} else if (obj.toString().startsWith(
						TypeBlockManager.QUOTE_LABEL)) {
					String[] quote = obj.toString().split(
							TypeBlockManager.QUOTE_LABEL);
					TypeBlockManager.automateBlockInsertion(
							(TextualFactoryBlock) obj, quote[1]);
					// otherwise, don't pass a label in
				} else {
					TypeBlockManager
							.automateBlockInsertion((TextualFactoryBlock) obj);
				}
			}
		}
	}
	...
	/**
	 * Private helper class to provide the semantics for various listeners
	 * within the editor's TextField.
	 */
	private class EditorListener extends KeyAdapter implements
		...
		/**
		 * Document Listener. Whenever AutoCompletePanel.editor receives a new
		 * user-generated character, it must update AutoCompletePanel to reflect
		 * the new pattern.
		 */
		...
		public void focusLost(FocusEvent e) {
			if (e.getOppositeComponent() == null
					|| e.getOppositeComponent().equals(menu)) {
				return;
			}
			setVisible(false);
			revalidate();
		...
		public void keyPressed(KeyEvent e) {
			if (e.getKeyChar() == KeyEvent.VK_ENTER) {
				...
				revalidate();
				repaint();
				//AutoCompletePane should not know about 
	...
	/**
	 * Private helper class to provide the semantics for various listeners
	 * within the menu's JList.
	 */
	private class MenuListener extends MouseAdapter implements FocusListener,
			KeyListener {
		...
		public void focusLost(FocusEvent e) {
			if (e.getOppositeComponent() == null
					|| e.getOppositeComponent().equals(editor)) {
				return;
			}
			setVisible(false);
			revalidate();
			...
			if (index != -1) {
				...
				g.drawString(
						...
						(int) metrics.getStringBounds(
								this.getText().substring(0, index), g)
								.getWidth() + 2,
						this.getHeight() - metrics.getDescent());
			}
		}
