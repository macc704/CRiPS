...
/**
 * The TypeBlockManager primary serves to help users drop blocks manually into
 * the bock canvas through the keyboard. To achieve this, the TypeBlockManager
 * commands three distinct phases: Interfacing, Searching, Dropping.
 */
public class TypeBlockManager {
	...
	/**
	 * empty string for labels that already exist and shouldn't be altered to
	 * user's preference
	 **/
	...
	/**
	 * Enables singleton instance of TypeBLockManager. If manager is enabled,
	 * computation is done over user-generated input.
	 */
	...
	/**
	 * Disables the TypeBlockManager. When disabled, TypeBlockManager still
	 * consumes key inputs but never proceeds to Phase 2. That is, the
	 * user-generated pattern is never parsed.
	 * 
	 */
	...
	private static boolean isNullBlockInstance(Long blockID) {
		if (blockID == null) {
			...
			return true;
		} else if (RenderableBlock.getRenderableBlock(blockID).getBlockID()
				.equals(Block.NULL)) {
			return true;
		} else {
			return false;
	...
	/**
	 * @requires the current block with focus must exist with non-null ID in a
	 *           non-null widget with a non-null parent
	 * @modifies the current block with focus
	 * @effects removes the current block with focus and all its children from
	 *          the GUI and destroys the link between the block with focus and
	 *          it's parent block if one exists
	 */
	protected static void automateBlockDeletion() {
		if (TypeBlockManager.manager == null) {
			...
			if (!isNullBlockInstance(TypeBlockManager.manager.focusManager
					.getFocusBlockID())) {
	 ...
	 *           non-null widget with a non-null parent
	 * @modifies the current block with focus
	 * @effects removes the current block with focus and children from the GUI
	 *          and destroys the link between the block with focus and it's
	 *          parent block if one exist and children blocks if it has
	 *          childrens.
	 */
	private void deleteBlockAndChildren() {
		...
		// Do not delete null block references. Otherwise, get Block and
		// RenderableBlock instances.
		if (isNullBlockInstance(focusManager.getFocusBlockID())) {
			throw new RuntimeException(
					"TypeBlockManager: deleting a null block references.");
		}
		Block block = Block.getBlock(focusManager.getFocusBlockID());
		RenderableBlock renderable = RenderableBlock.getRenderableBlock(block
				.getBlockID());
		...
		if (widget == null) {
			throw new RuntimeException(
					"TypeBlockManager: do not delete blocks with no parent widget.");
			// return;
		}
		...
		Point location = SwingUtilities.convertPoint(renderable,
				new Point(0, 0), this.blockCanvas.getCanvas());
		...
		if (validConnection(block.getPlug())) {
			...
			BlockConnector parentConnectorToBlock = Block.getBlock(parentID)
					.getConnectorTo(block.getBlockID());
			this.disconnectBlock(block, widget);
			if (validConnection(block.getAfterConnector())) {
				Long afterBlockID = block.getAfterBlockID();
				disconnectBlock(Block.getBlock(afterBlockID), widget);
				if (parentID != null) {
					BlockLink link = BlockLinkChecker.canLink(Block
							.getBlock(parentID), Block.getBlock(afterBlockID),
							parentConnectorToBlock, Block
									.getBlock(afterBlockID)
									.getBeforeConnector());
					if (link != null) {
						link.connect();
						...
						RenderableBlock.getRenderableBlock(
								link.getPlugBlockID()).repaintBlock();
						RenderableBlock.getRenderableBlock(
								link.getPlugBlockID()).repaint();
						RenderableBlock.getRenderableBlock(
								link.getPlugBlockID()).moveConnectedBlocks();
						RenderableBlock.getRenderableBlock(
								link.getSocketBlockID()).repaintBlock();
						RenderableBlock.getRenderableBlock(
								link.getSocketBlockID()).repaint();
	...
	private void removeChildrenBlock(RenderableBlock renderable,
			WorkspaceWidget widget, Container container) {
		...
		// renderable.getBlockID(), WorkspaceEvent.BLOCK_REMOVED));
		for (BlockConnector child : Block.getBlock(renderable.getBlockID())
				.getSockets()) {
			if (child == null || child.getBlockID().equals(Block.NULL))
				continue;
			RenderableBlock childRenderable = RenderableBlock
					.getRenderableBlock(child.getBlockID());
		...
		if (Block.getBlock(renderable.getBlockID()).isProcedureDeclBlock()) {
			if (Block.getBlock(renderable.getBlockID()).getAfterBlockID() != Block.NULL) {
				removeAfterBlock(
						RenderableBlock.getRenderableBlock(Block.getBlock(
								renderable.getBlockID()).getAfterBlockID()),
						widget, container);
				this.disconnectBlock(Block.getBlock(Block.getBlock(
						renderable.getBlockID()).getAfterBlockID()), widget);
			}
		}
		if (renderable.hasComment())
			renderable.removeComment();
		Workspace.getInstance().notifyListeners(
				new WorkspaceEvent(widget, renderable.getBlockID(),
						WorkspaceEvent.BLOCK_REMOVED));
	}

	/**
	 * Helper method that recursively finds and removes all the blocks connected
	 * to the bottom of this block, including this block.
	 * 
	 * @param afterBlock
	 *            - RenderableBlock we start removing at
	 * @param widget
	 *            - WorkspaceWidget that the block is using
	 * @param container
	 *            - Container the block is stored in
	 * 
	 */

	private void removeAfterBlock(RenderableBlock afterBlock,
			WorkspaceWidget widget, Container container) {
		if (Block.getBlock(afterBlock.getBlockID()).getAfterBlockID() != Block.NULL) {
			removeAfterBlock(
					RenderableBlock.getRenderableBlock(Block.getBlock(
							afterBlock.getBlockID()).getAfterBlockID()),
					widget, container);
		}
	 ...
	 * @param container
	 * 
	 * @requries renderable != null && renderable.blockID != null &&
	 *           renderable.blockID != Block.NULL && widget != null && container
	 *           != null
	 ...
	 *          renderable block, widget, and container appropriately. Repeats
	 *          for all of renderable's children.
	 */
	private void removeBlock(RenderableBlock renderable,
			WorkspaceWidget widget, Container container) {
		widget.removeBlock(renderable);
		container.remove(renderable);
		container.validate();
		...
		// Workspace.getInstance().notifyListeners(new WorkspaceEvent(widget,
		// renderable.getBlockID(), WorkspaceEvent.BLOCK_REMOVED));
		for (BlockConnector child : BlockLinkChecker.getSocketEquivalents(Block
				.getBlock(renderable.getBlockID()))) {
			if (child == null || child.getBlockID().equals(Block.NULL))
				continue;
			RenderableBlock childRenderable = RenderableBlock
					.getRenderableBlock(child.getBlockID());
	 ...
	 * @modifies
	 * @effects Does nothing if: childBlock is invalid (null) Otherwise, remove
	 *          childBlock from it's parent block if the childBlock has a
	 *          parent. If it does not have a parent, do nothing.
	 */
	private void disconnectBlock(Block childBlock, WorkspaceWidget widget) {
		if (childBlock == null || invalidBlockID(childBlock.getBlockID()))
			return;
		BlockConnector childPlug = BlockLinkChecker
				.getPlugEquivalent(childBlock);
		if (childPlug == null || !childPlug.hasBlock()
				|| isNullBlockInstance(childPlug.getBlockID()))
			return;
		Block parentBlock = Block.getBlock(childPlug.getBlockID());
		BlockConnector parentSocket = parentBlock.getConnectorTo(childBlock
				.getBlockID());
		if (parentSocket == null)
			return;
		// disconector if child connector exists and has a block connected to it
		BlockLink link = BlockLink.getBlockLink(childBlock, parentBlock,
				childPlug, parentSocket);
		if (link == null)
			return;

		link.disconnect();

		RenderableBlock parentRenderable = RenderableBlock
	 ...
	 * @effects change bufferedBlock such that it points to the block with
	 *          current focus
	 */
	...
	protected static void automateCopyBlock() {
		...
		TypeBlockManager.manager.bufferedBlock = BlockUtilities
				.makeNodeWithChildren(TypeBlockManager.manager.focusManager
						.getFocusBlockID());
	}
	...
	protected static void automateCopyAll() {
		...
		TypeBlockManager.manager.bufferedBlock = BlockUtilities
				.makeNodeWithStack(TypeBlockManager.manager.focusManager
						.getFocusBlockID());
	}
	...
	protected static void automatePasteBlock() {
		...
		TypeBlockManager.manager
				.pasteStack(TypeBlockManager.manager.bufferedBlock);
	}
	...
	private void pasteStack(BlockNode node) {
		...
		if (invalidBlockID(focusManager.getFocusBlockID())) {
			...
			Point location = SwingUtilities
					.convertPoint(this.blockCanvas.getCanvas(),
							this.focusManager.getCanvasPoint(),
							Workspace.getInstance());
			widget = Workspace.getInstance().getWidgetAt(location);
			spot = SwingUtilities.convertPoint(this.blockCanvas.getCanvas(),
					this.focusManager.getCanvasPoint(), widget.getJComponent());
		} else {
			RenderableBlock focusRenderable = RenderableBlock
		...
		if (widget == null) {
			// To be examined and fixed, occurs on macs
			JOptionPane.showMessageDialog(TypeBlockManager.manager.frame,
					"Please click somewhere on the canvas first.", "Error",
					JOptionPane.PLAIN_MESSAGE);
			// throw new
			// RuntimeException("Why are we adding a block to a null widget?");
		} else {
			...
			if (BlockUtilities.blockExists(node)) {
				...
				RenderableBlock mirror = BlockUtilities.makeRenderable(node,
						widget);
				mirror.setLocation(spot);
				mirror.moveConnectedBlocks(); // make sure the childrens are
												// placed correctly
			} else {
				/*
				 * widgets = Workspace.getInstance().getWorkspaceWidgets(); for
				 * (WorkspaceWidget widg: widgets){ if (widg instanceof Page){
				 * widget = widg; break; } }
				 */
				// future version, allow them to paste
				JOptionPane
						.showMessageDialog(
								TypeBlockManager.manager.frame,
	...
	/**
	 * Traverses the block tree structure to move in the direction of the input
	 * argument.
	 * 
	 * @param dir
	 */
	...
	private void traverseFocus(Direction dir) {
		if (isNullBlockInstance(focusManager.getFocusBlockID())) {
			if (dir == Direction.UP) {
				blockCanvas.getVerticalModel().setValue(
						blockCanvas.getVerticalModel().getValue() - 5);
			} else if (dir == Direction.DOWN) {
				blockCanvas.getVerticalModel().setValue(
						blockCanvas.getVerticalModel().getValue() + 5);
			} else if (dir == Direction.LEFT) {
				blockCanvas.getHorizontalModel().setValue(
						blockCanvas.getHorizontalModel().getValue() - 5);
			...
			if (dir == Direction.UP) {
				...
				RenderableBlock block = RenderableBlock
						.getRenderableBlock(focusManager.getFocusBlockID());
				Point location = SwingUtilities.convertPoint(block, new Point(
						0, 0), this.blockCanvas.getCanvas());
				this.focusManager.setFocus(location, Block.NULL);
				this.blockCanvas.getCanvas().requestFocus();
			} else if (dir == Direction.ENTER) {
				RenderableBlock.getRenderableBlock(
	...
	private void displayAutoCompletePanel(char character) {
		...
		if (invalidBlockID(focusManager.getFocusBlockID())) {
			// canvas has focus
			this.blockCanvas.getCanvas().add(autoCompletePanel,
			...
			RenderableBlock block = RenderableBlock
					.getRenderableBlock(focusManager.getFocusBlockID());
			Point location = SwingUtilities.convertPoint(block,
	...
	/**
	 * assumes number and differen genus exist and number genus has ediitabel
	 * lable
	 */
	protected static void automateNegationInsertion() {
		...
		// get focus block
		Long parentBlockID = TypeBlockManager.manager.focusManager
				.getFocusBlockID();
	...
	protected static void automateMultiplication(char character) {
		...
		if (!isNullBlockInstance(TypeBlockManager.manager.focusManager
			...
			Block parentBlock = Block
					.getBlock(TypeBlockManager.manager.focusManager
							.getFocusBlockID());
	 ...
	 * @param label
	 * 
	 * @requires if (label != null) then associated block.isLabelEditable()
	 *           should return true
	 * @modifies focusManager.focusblock && focusManager.focuspoint &&
	 *           blockCanvas
	 * @effects Do nothing if "genusName" does not map to a valid block.
	 *          Otherwise, create and add a new block with matching genus and
	 *          label properties to one of the following: 1. the current block
	 *          with focus at (0,0) relative to that block. 2. the current block
	 *          with focus at next applicable socket location 3. the canvas at
	 *          the last mouse click point. Then update any focus and block
	 *          connections.
	 */
	protected static void automateBlockInsertion(String genusName, String label) {
		...
		if (createdRB == null) {
			...
			if (label != null
					&& Block.getBlock(createdRB.getBlockID()).isLabelEditable()
					&& !Block.getBlock(createdRB.getBlockID())
							.labelMustBeUnique()) {
	...
	protected static void automateBlockInsertion(TextualFactoryBlock block) {
		/*
		 * Passing in an empty label name means that the block should already
		 * have a predetermined label name that does not need to be altered to
	 ...
	 *          with focus at (0,0) relative to that block. 2. the current block
	 *          with focus at next applicable socket location 3. the canvas at
	 *          the last mouse click point. If label is not an empty string,
	 *          then set the block label to that string. Then update any focus
	...
	protected static void automateBlockInsertion(TextualFactoryBlock block,
		...
		// sets the label of the block to whatever the user typed (should only
		// be numbers)
	...
	/**
	 * @param block
	 *            - the textual block from which a new RenderableBlock will be
	 *            constructed
	 * 
	 * @requires
	 * @modifies nothing
	 * @effects none
	 * @return new RenderableBlock instance from the TextualFactoryBlock or null
	 *         if not possible.
	 */
	private static RenderableBlock createRenderableBlock(
			TextualFactoryBlock block) {
		// if textual wrapper is null, return a null instance of
		// RenderableBlock.
		if (block == null)
			return null;
		// if FactoryBlock wrapped in textual wrapper is invalid, return null
		// RenderableBlock instance.
		if (block.getfactoryBlock() == null
				|| block.getfactoryBlock().getBlockID().equals(Block.NULL))
			return null;
		// create and get the RenderableBloc instance associated with the
		// Textual wrapper's FactoryBlock
		RenderableBlock createdRB = block.getfactoryBlock().createNewInstance();
		// if the above instance of RenderableBlock is invalid (null or points
		// to null)
	 ...
	 * @param block
	 * 
	 * @requires block must be a valid block. That is, block may not be such
	 *           that block == null || block.getBlockID() == null ||
	...
	private void addBlock(RenderableBlock block) {
		...
		if (invalidBlockID(parentBlockID)) {
			new BlockDropAnimator(this.focusManager.getCanvasPoint(), block,
			...
			new BlockDropAnimator(SwingUtilities.convertPoint(parentBlock,
					this.focusManager.getBlockPoint(),
					this.blockCanvas.getCanvas()), block,
					RenderableBlock.getRenderableBlock(parentBlockID));
		}
